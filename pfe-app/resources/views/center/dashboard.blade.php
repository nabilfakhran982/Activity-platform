                ...
                if (res.ok) {
                    ...
                    if (slide) {
                        ...
                        setTimeout(() => {
                            ...
                            // تحديث الـ pending count بالـ badge
                            const badge = document.querySelector('.font-display.text-2xl span');
                            if (badge) {
                            ...
                            if (pendingStat) {
                                const current = parseInt(pendingStat.textContent) || 0;
                                pendingStat.textContent = Math.max(0, current - 1);
                            }

                            // إذا ما في pending — شيل الـ section
                            const track = document.getElementById('pending-track');
                            if (track && track.children.length === 0) {
                                track.closest('.mb-12').remove();
                            }
                        }, 300);
                    }
                }
