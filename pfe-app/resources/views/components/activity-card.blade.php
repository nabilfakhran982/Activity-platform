...
<div class="activity-card">
    ...
    <div class="image-box {{ !$image ? $bg : '' }}" style="position:relative">
        ...
        {{-- Favourite button --}}
        @auth
            <button onclick="toggleFavourite({{ $act->id }}, this)" class="fav-btn {{ $isSaved ? 'saved' : '' }}"
    ...
    <div class="p-5">
        ...
        <p class="text-xs mb-3" style="color:#8a7a6a">{{ $act->center->name }} · {{ $act->center->address }} ·
            {{ $act->center->city }}</p>
        ...
        <div class="flex items-center justify-between pt-3 border-t border-[#F0EDE6]">
            ...
            <div class="flex items-center gap-2">
                ...
                <a href="{{ route('activity.show', $act->id) }}" class="search-btn px-4 py-2 text-xs">
                    View & Book
                </a>
            </div>
        </div>
