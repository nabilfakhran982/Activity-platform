...
function removeScheduleRow(btn) {
    ...
    Array.from(container.children).forEach((row, i) => {
        row.querySelector('[name*="day_of_week"]').name =
            `schedules[${i}][day_of_week]`;
        row.querySelector('[name*="start_time"]').name =
            `schedules[${i}][start_time]`;
        row.querySelector('[name*="end_time"]').name =
...
function updateActivityInGrid(id, act) {
    ...
    if (imgWrapper) {
        const overlay = imgWrapper.querySelector(
            ".activity-actions-overlay",
        ).outerHTML;
    ...
    if (body) {
        ...
        const schedulesHtml =
            act.schedules && act.schedules.length > 0
                ? `<div class="schedules-list">${act.schedules
                      .map(
                          (s) => `
                <div class="schedule-item">
                    <span class="schedule-day">${s.day_of_week.charAt(0).toUpperCase() + s.day_of_week.slice(1)}</span>
                    <span class="schedule-time">${s.start_time.substring(0, 5)} – ${s.end_time.substring(0, 5)}</span>
                </div>`,
                      )
                      .join("")}</div>`
    ...
    <div style="background:#fff;border-radius:24px;padding:40px 32px;max-width:360px;width:100%;text-align:center">
        ...
        <div style="display:flex;gap:10px">
            <button onclick="document.getElementById('delete-activity-modal-${id}').remove()"
                style="flex:1;padding:12px;border:1px solid #E8E5DF;border-radius:999px;background:#fff;cursor:pointer;font-size:13px;font-weight:500;color:#5a5751;font-family:'DM Sans',sans-serif;transition:border-color 0.2s"
            ...
            <button onclick="confirmDeleteActivity(${id})"
                style="flex:1;padding:12px;border:none;border-radius:999px;background:#e05252;color:#fff;cursor:pointer;font-size:13px;font-weight:500;font-family:'DM Sans',sans-serif;transition:background 0.2s"
    ...
    if (res.ok) {
        ...
        if (card) {
            ...
            setTimeout(() => {
                ...
                if (
                    grid &&
                    grid.querySelectorAll('[id^="activity-card-"]').length === 0
                ) {
                    grid.innerHTML = `
